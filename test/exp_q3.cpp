...
void q3_dp_ops(std::map<string, SecureRelation*> rels_dict,std::map<string, std::vector<Stats>> rel_stats_dict){
    ...
    CostModel cm;
    float cost = 0;
    std::tuple<float,float> costModel_output = cm.get_cost(jNode3,cost, 1);
    cost = std::get<0>(costModel_output);
    std::cout << "Q3 cost DP Syscat case (cost model output): " << cost << endl;
    ...
    std::cout << "\n\nQ3 runtime with DP Syscat: " << duration_filter_by_fixed_value/5 << " ms" << std::endl;
}
...
void q3_prev_ops(std::map<string, SecureRelation*> rels_dict){
    ...
    CostModel cm;
    float cost = 0;
    std::tuple<float,float> costModel_output = cm.get_cost(jNode3, cost, 0);
    float costModel_cost = std::get<0>(costModel_output);
    std::cout << "Q3 CostModel cost oblivious case: " << costModel_cost << endl;
    ...
    auto end_time = std::chrono::high_resolution_clock::now();
    auto duration_filter_by_fixed_value = std::chrono::duration_cast<std::chrono::milliseconds>((end_time - start_time)).count();
    std::cout << "\n\nQ3 runtime oblivious case: " << duration_filter_by_fixed_value/5 << " ms" << std::endl;
}

void debug(std::map<string, SecureRelation*> rels_dict){
    planNode *r1 = new planNode(rels_dict["rel1"]);
    r1->node_name = "rel1";
    planNode *r2 = new planNode(rels_dict["rel2"]);
    r2->node_name = "rel2";
    EquiJoinOperator* jOp_inter1 = new EquiJoinOperator(1, 1);
    planNode *jNode_inter1 = new planNode(jOp_inter1, r1, r2, 1);
    jNode_inter1->node_name = "join_inter1";
    jNode_inter1->set_previous(*r1,1);
    jNode_inter1->set_previous(*r2,2);

    planNode *r3 = new planNode(rels_dict["rel3"]);
    r3->node_name = "rel3";
    EquiJoinOperator* jOp_inter2 = new EquiJoinOperator(1, 1);
    planNode *jNode_inter2 = new planNode(jOp_inter2, jNode_inter1, r3, 1);
    jNode_inter2->node_name = "join_inter2";
    jNode_inter2->set_previous(*jNode_inter1,1);
    jNode_inter2->set_previous(*r3,2);


    planNode *testNode = new planNode(rels_dict["rel4"]);
    testNode->node_name = "rel4";

    EquiJoinOperator* jOp_test = new EquiJoinOperator(1, 1);
    planNode* jNode_test = new planNode(jOp_test, jNode_inter2, testNode, 1);
    jNode_test->set_previous(*jNode_inter2,1);
    jNode_test->set_previous(*testNode,2);
    SecureRelation* qout_test = jNode_test->get_output();
    std::cout << qout_test->flags.size() << endl;
}
...
int main(int argc, char** argv) {
    ...
    get_noisy_relation_statistics(rels_dict, num_cols, party, rel_stats_dict);
