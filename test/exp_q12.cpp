...
void q1_dp_ops(std::map<string, SecureRelation*> rels_dict,std::map<string, std::vector<Stats>> rel_stats_dict){
    ...
    planNode* jNode = new planNode(jOp, filterNode1, testNode2, jOp->selectivity);
    ...
    CostModel cm;
    float cost = 0;
    std::tuple<float,float> costModel_output = cm.get_cost(jNode,cost, 1);
    cost = std::get<0>(costModel_output);
    std::cout << "Q1 cost DP Syscat case (cost model output): " << cost << endl;
...
void q1_prev_ops(std::map<string, SecureRelation*> rels_dict){
    ...
    planNode* jNode = new planNode(jOp, filterNode1, testNode2, 1);
...
void q1_dp_ops_cost(std::map<string, SecureRelation*> rels_dict,std::map<string, std::vector<Stats>> rel_stats_dict){
    planNode *testNode = new planNode(rels_dict["rel1"]);
    testNode->node_name = "rel1";
    ...
    float input_size1 = rels_dict["rel1"]->flags.size();
    float input_size2 = rels_dict["rel2"]->flags.size();
    float f1_cost = 2*input_size1 + std::ceil(input_size1 * pow(std::log10(input_size1),2));
    float f2_cost = 2*input_size2 + std::ceil(input_size2 * pow(std::log10(input_size2),2));
    float j_size = filterNode1->selectivity * input_size1 * filterNode2->selectivity * input_size2;
    float j_cost = j_size + std::ceil(j_size * pow(std::log10(j_size),2));
    float manual_cost = f1_cost + f2_cost + j_cost;
    std::cout << "Q1 cost DP Syscat case (manual output): " << manual_cost << endl;
    // cost model cost computation 
    CostModel cm;
    float cost = 0;
    std::tuple<float,float> costModel_output = cm.get_cost(jNode,cost, 1);
...
int main(int argc, char** argv) {
   ...
   std::cout << "\n\n___________________________________________________________________________________________________________________________\n\n";
    ...
    //std::cout << "Q1 cost model check (oblivious):\n";
    //q1_prev_ops_cost(rels_dict);

    // testing the cost model
    //std::cout << "Q1 cost model check (DP):\n";
    //q1_dp_ops_cost(rels_dict, rel_stats_dict);
